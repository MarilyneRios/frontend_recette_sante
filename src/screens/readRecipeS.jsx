      ...
      {isSuccess &&
        currentRecipes.map((recipe) => (
          ...
          style={{ width: "16rem", position: "relative" }}>
            ...
            <Card.Img variant="top" src={recipe.imageUrl || bookImage} className="mx-5 mt-2 w-50" />
            <Card.Body>
              <Card.Title className="text-center"> {recipe.name}</Card.Title>
              <Card.Text  className="text-center"> {recipe.category}</Card.Text>
              <Card.Text  className="text-center">De {recipe.pseudo}</Card.Text>
            </Card.Body>
          </Card>
          </div>
          </div>
